...
set all_projects=xunit.fluently.assert
set all_projects=%all_projects%%delim%xunit.fluently.assert.exceptionally
set all_projects=%all_projects%%delim%xunit.fluently.assert.ioly
set all_projects=%all_projects%%delim%xunit.fluently.theoretically
set all_projects=%all_projects%%delim%xunit.fluently.theoretically.sample
rem Setup Assert
set assert_projects=xunit.fluently.assert
rem Setup Fluently Exceptionally
set except_projects=xunit.fluently.assert.exceptionally
rem Setup Fluently I/O-ly
set io_projects=xunit.fluently.assert.ioly
rem Setup Fluently Theoretically
...
if "%1" == "--assert" (
    if "%projects%" == "" (
        set projects=%assert_projects%
    ) else (
        set projects=%projects%%delim%%assert_projects%
    )
	goto :next_arg
)

:add_exceptionally_projects
if "%1" == "--except" (
    if "%projects%" == "" (
        set projects=%except_projects%
    ) else (
        set projects=%projects%%delim%%except_projects%
    )
	goto :next_arg
)
if "%1" == "--exceptionally" (
...
if "%1" == "--io" (
    if "%projects%" == "" (
        set projects=%io_projects%
    ) else (
        set projects=%projects%%delim%%io_projects%
    )
	goto :next_arg
)
if "%1" == "--ioly" (
...
:add_theoretically_projects
if "%1" == "--theory" (
    if "%projects%" == "" (
        set projects=%theory_projects%
    ) else (
        set projects=%projects%%delim%%theory_projects%
    )
    goto :next_arg
)
if "%1" == "--theoretically" (
...
set xcopy_opts=/y /f
rem Because our local packages location changed
